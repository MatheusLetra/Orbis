    ...
    await tx`insert into users (id, email, name, password_hash, is_active, created_at, updated_at)
     ...
     await tx`insert into system_versions (id, company_id, system_id, version, is_active, created_at, updated_at)
      ...
      await tx`insert into memberships (id, company_id, user_id, position, permissions, is_active, created_at, updated_at)
             values (${fixture.actorMembershipA}, ${fixture.companyA}, ${fixture.actorId}, 'GESTOR', ${JSON.stringify(["tasks.create", "tasks.update", "tasks.read", "kanban.manage", "hours.register", "capacity.read", "company.read", "company.update", "users.read", "users.manage", "permissions.manage", "systems.read", "systems.manage", "versions.manage", "requisitions.read", "requisitions.create", "requisitions.update", "requisitions.delete", "chat.use", "audit.read", "releases.read", "releases.manage"])}, true, ${now}, ${now}),
